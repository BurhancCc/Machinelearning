...
# the hyperparameters we are tuning are the penalty, crossvalidation, and the solver
# the training set is split into the a training and validation part my the GridSearchCV method
# you could increase the C parameter to increase the accuracy
# but the training of the model and the hyperparameter tuning starts taking a really long time
# the hyperparameter tuning part takes rougly 10 minutes to run
"""
...
"""
"""
# after hyper parameter tuning these are the hyper parameters that came out of it C = 100000, penalty = l2, solver = newton-cg
# the higher the c value the better the model became but at some point it started take way to long and the improvement of the model became really limited
logreg=LogisticRegression(C=100000, penalty="l2", solver = 'newton-cg')
...
class_names=["Above_Average", "Below_Average", "Fairly_Cheap", "Fairly_Expensive", "Really_Cheap",  "Really_Expensive"] # name  of classes
fig, ax = plt.subplots()
...
target_names = ["Above_Average", "Below_Average", "Fairly_Cheap", "Fairly_Expensive", "Really_Cheap",  "Really_Expensive"]
...
# this prints the best hyperparameter combination
print("Best: %f using %s" % (grid_result.best_score_, grid_result.best_params_))
print(grid_result.best_params_['var_smoothing'])
means = grid_result.cv_results_['mean_test_score']
stds = grid_result.cv_results_['std_test_score']
params = grid_result.cv_results_['params']
...
# fit the model with data and the hyper parameters we found
clfNaiveBayes = GaussianNB(var_smoothing = grid_result.best_params_['var_smoothing'])
clfNaiveBayes.fit(X_train, y_train)

print("Score: ", clfNaiveBayes.score(X_test, y_test))

y_pred = clfNaiveBayes.predict(X_test)
...
### K-nearest neighbors
def findOptimalKValue():
    # here we are applying the grid_search hyperparameter tuning method to a K-nearest neighbors model
    # the hyperparameters we are tuning are the K-value, the weights, and the metric
    # the training set is split into the a training and validation part by the GridSearchCV method
    # the hyperparameter tuning part takes roughly 2 minutes to run
...
# Running the aforementioned function to determine the optimal hyperparameters
#findOptimalKValue()

# After hyper parameter tuning, optimal parameters were shown as below
...
cnf_matrix = confusion_matrix(y_test, y_test_knn)
print(cnf_matrix)
...
def findOptimalSVMParameters():
    # here we are applying the grid_search hyperparameter tuning method to a support vector machine model
    # the hyperparameters we are tuning are as follows: gamma, and kernel
    # the training set is split into the a training and validation part by the GridSearchCV method
    # the hyperparameter tuning part takes roughly 20 minutes to run
...
# Evaluating the accuracy of our SVM model
cnf_matrix = confusion_matrix(y_test, y_test_svc)
print(cnf_matrix)
print(classification_report(y_test, y_test_svc))
print('Test set accuracy: ', metrics.accuracy_score(y_test, y_test_svc))
