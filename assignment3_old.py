...
# Testing different amount of clusters to look at the log-likelihood, BIC, and AIC.
...
plt.ylabel("SSE")
plt.show()
# Graphs showing the elbow graphs to pick the amount of clusters for EM (multiple graphs because it's visually unclear on a single graph)
...
# For EM, the KneeLocator tells us that 6 clusters is optimal here, and the eyetest says 5 or 6 clusters.
...
# Initiate k-means & EM algorithms
...
high_probability_indexes = [x.index for x in predicted_probabilities_list if max(x) > 0.7]
...
print("KMEANS CENTERS KMEANS CENTERS KMEANS CENTERS")
...
fig, ax = plt.subplots()
sn.heatmap(pd.DataFrame(em_model.means_), annot=True, cmap="YlGnBu" ,fmt='g')
ax.xaxis.set_label_position("top")
ax.set_xticks(np.arange(len(column_names)), labels=column_names)
plt.xlabel('column names')
plt.show()
...
# The EM plot
em_colors = [ fte_colors[label] for label in em_labels ]
ax2.scatter( scaled[:, 1], scaled[:, 3], c=em_colors, vmax = 0.8 )
